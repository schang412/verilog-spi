...
class TB(object):
    def __init__(self, dut):
        ...
        self.source = BufferSource(dut.clk, dut.sclk, dut.rxd, bits=int(os.environ["PARAM_DATA_WIDTH"]), sclk_pol=spi_mode_cpol_map[os.environ["PARAM_SPI_MODE"]], sclk_phase=spi_mode_cpha_map[os.environ["PARAM_SPI_MODE"]])
...
async def run_test(dut, payload_data=None, payload_lengths=None):
    tb = TB(dut)
    await tb.reset()
...
@pytest.mark.parametrize("spi_mode", [0, 1, 2, 3])
@pytest.mark.parametrize("data_width", [8, 16, 32])
def test_spi_tx(request, data_width, spi_mode):
    dut = "spi_rx"
